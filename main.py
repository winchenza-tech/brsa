...
def process_ocr(image_path):
    ...
    logger.info("OCR İşlendi.")
    # Gerçek OCR sonucunu parse etmeli; şimdilik placeholder:
    return "<div>İŞ YATIRIM: 1.50M</div>", "<div>GARANTİ: 1.20M</div>"

# --- 6. KOMUTLAR ---
# ❌ ESKİ: filters.bot ekliydi → kullanıcı mesajlarını görünmez kılıyordu
# ✅ YENİ: filters.bot kaldırıldı

@bot_app.on_message(filters.command(["test"]))
async def test_cmd(client, message):
    logger.info(f"👉 /test komutu alındı! Kullanıcı: {message.from_user.id}")
    await message.reply_text("✅ Sistem çalışıyor! Bağlantı başarılı.")

@bot_app.on_message(filters.command(["calısmiyor", "çalışmıyor"]))
async def debug_cmd(client, message):
    await message.reply_text(
        "🛠 Sistem durumu kontrol ediliyor... "
        "Loglara bakıyorum. Hata alıyorsan ekran görüntüsü alıp geliştiriciye ilet."
    )
...
async def handle_request(client, message):
    ...
    try:
        # Userbotu ile hedef bota mesaj gönder
        await user_app.send_message(TARGET_BOT, f"/akd {hisse}")
        await asyncio.sleep(5)

        # Gelen son mesajı kontrol et
        ...
        await wait_msg.edit("❌ Hedef bottan resim gelmedi.")
...
async def start_systems():
    ...
    await bot_app.start()
    logger.info("✅ Ana bot başlatıldı.")
