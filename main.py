...
# --- 1. LOGLARI ANLIK AKIŞA ALMA (ÇÖZÜLDÜ) ---
...
TELEGRAM_TOKEN = os.environ.get("TELEGRAM_TOKEN", "") # Railway Variables'taki isminle aynı
...
def process_ocr(image_path):
    ...
    logger.info(f"OCR Analizi Tamamlandı.")
    # Burayı kendi veri parse mantığınla doldurabilirsin
...
async def kontrol_test(client, message):
    await message.reply_text("✅ Bot aktif ve çalışıyor!")
...
async def handle_request(client, message):
    ...
    try:
        await user_app.send_message(TARGET_BOT, f"/{message.command[0]} {hisse}")
...
async def start_systems():
    logger.info("🚀 Ana sistem başlatılıyor...")
    await bot_app.start()
